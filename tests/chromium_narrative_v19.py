...
profile = Path('/tmp/fb-v19-cdp')
shutil.rmtree(profile, ignore_errors=True)
server = None
chrome = subprocess.Popen(
    [
        'chromium', '--headless=new', '--no-sandbox', '--disable-gpu',
        '--remote-debugging-port=9344', '--remote-allow-origins=*',
...
try:
    pages = wait_json('http://127.0.0.1:9344/json/list')
    page = next((item for item in pages if item.get('type') == 'page'), pages[0])
    ws = websocket.create_connection(
        page['webSocketDebuggerUrl'], timeout=60, origin='http://127.0.0.1:9344'
    )
    seq = 0
    events = []
    ...
    checks = {}
    checks['title_v19'] = ev("document.title.includes('Narrative 1.9')")
    checks['data_v19'] = ev(
        "window.GAME_DATA.meta.version==='1.9.0' && window.GAME_DATA.nodes.length===700"
    )
    checks['save_namespace'] = ev(
        "FB_DEBUG.saveKeys().slot1.includes('production-v19-slot1')"
    )
    ...
    report = {
        ...
        'method': 'Chromium CDP injected-page validation (navigation blocked by administrator policy)',
        'checks': checks,
        'exceptions': exceptions,
        'console_errors': logs,
        'screenshot': 'narrative_v19_mobile.png',
    }
    (TESTS / 'browser_narrative_v19.json').write_text(
